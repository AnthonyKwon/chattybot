...
async function commandHandler(interaction) {
    ...
    if (!newLocale) {
        ...
        interaction.editReply(i18n.get(interaction.locale, 'message.discord.locale.guild_current')
    ...
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
        interaction.editReply(i18n.get(interaction.locale, 'error.discord.user.no_permission'));
        return;
    }
    ...
    if (!interaction.guild.members.me.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
        interaction.editReply(i18n.get(interaction.locale, 'error.discord.bot.no_permission')
    ...
    if (!Object.values(Locale).includes(newLocale)) {
        interaction.editReply(i18n.get(interaction.locale, 'error.discord.command.locale_invalid_params'));
        return;
    }

    // check if server locale is the same as before
    if (interaction.guild.preferredLocale === newLocale) {
        interaction.editReply(i18n.get(interaction.locale, 'error.discord.command.locale_already_set'))
        return;
    }

    // set parameter as new guild locale
    interaction.guild.edit({ preferredLocale: newLocale });
    interaction.editReply(i18n.get(interaction.locale, 'message.discord.locale.guild_set')
